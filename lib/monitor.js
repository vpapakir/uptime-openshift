...
Monitor.prototype.findChecksNeedingPoll = function(callback) {
  var options = url.parse('http://' + process.env.OPENSHIFT_APP_DNS + ':' + process.env.OPENSHIFT_NODEJS_PORT +'/api/checks/needingPoll');
  this.applyApiHttpOptions(options);
  var self = this;
  http.get(options, function(res) {
    if (res.statusCode != 200) {
      return callback(new Error(process.env.OPENSHIFT_APP_DNS + ':' + process.env.OPENSHIFT_NODEJS_PORT +'/api/checks/needingPoll resource responded with error code: ' + res.statusCode));
    }
    var body = '';
    ...
    callback(new Error(process.env.OPENSHIFT_APP_DNS + ':' + process.env.OPENSHIFT_NODEJS_PORT +'/api/checks/needingPoll resource not available: ' + e.message));
  });
};
...
Monitor.prototype.declarePoll = function(check, callback) {
  var options = url.parse('http://' + process.env.OPENSHIFT_APP_DNS + ':' + process.env.OPENSHIFT_NODEJS_PORT +'/api/check/' + check._id + '/test');
  options.method = 'PUT';
  this.applyApiHttpOptions(options);
  var self = this;
  var req = http.request(options, function(res) {
    if (res.statusCode != 200) {
      return callback(new Error(process.env.OPENSHIFT_APP_DNS + ':' + process.env.OPENSHIFT_NODEJS_PORT +'/api/check/:id/test resource responded with error code: ' + res.statusCode));
    }
    res.on('data', function(chunk) {
      // do nothing
    });
    ...
    callback(new Error(process.env.OPENSHIFT_APP_DNS + ':' + process.env.OPENSHIFT_NODEJS_PORT +'/api/check/:id/test resource not available: ' + e.message));
  });
  req.end();
};
...
Monitor.prototype.createPing = function(error, check, timestamp, time, details, callback) {
  ...
  var options = url.parse('http://' + process.env.OPENSHIFT_APP_DNS + ':' + process.env.OPENSHIFT_NODEJS_PORT +'/api/pings');
  options.method = 'POST';
  options.headers = {
  ...
  var req = http.request(options, function(res) {
    if (res.statusCode != 200) {
      return callback(new Error(process.env.OPENSHIFT_APP_DNS + ':' + process.env.OPENSHIFT_NODEJS_PORT +'/api/pings resource responded with error code: ' + res.statusCode));
    }
    var body = '';
    res.on('data', function(chunk) {
    body += chunk;
    ...
    callback(new Error(process.env.OPENSHIFT_APP_DNS + ':' + process.env.OPENSHIFT_NODEJS_PORT +'/api/pings resource not available: ' + e.message));
  });
  req.write(postData);
  req.end();
};
