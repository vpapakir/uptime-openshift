...
Monitor.prototype.findChecksNeedingPoll = function(callback) {
  var options = url.parse(process.env.OPENSHIFT_APP_DNS + ':' + process.env.OPENSHIFT_NODEJS_PORT +'/api/checks/needingPoll');
...
Monitor.prototype.declarePoll = function(check, callback) {
  var options = url.parse(process.env.OPENSHIFT_APP_DNS + ':' + process.env.OPENSHIFT_NODEJS_PORT +'/api/check/' + check._id + '/test');
  options.method = 'PUT';
  this.applyApiHttpOptions(options);
  var self = this;
...
Monitor.prototype.createPing = function(error, check, timestamp, time, details, callback) {
  ...
  var options = url.parse(process.env.OPENSHIFT_APP_DNS + ':' + process.env.OPENSHIFT_NODEJS_PORT +'/api/pings');
  options.method = 'POST';
  options.headers = {
